# 👋 Hi, I'm Minjoo Kim

- 🏗️ Data Engineer at **LINE+**, building and operating large-scale data platforms (ad pipelines serving 130M+ DAU).
- 🤖 Currently focused on **AI agent engineering** — designing production-minded GenAI / agentic systems with **LangGraph**, **Graph RAG**, and LLM observability.
- 🔒 My core production code is private under NDA. The **pinned repositories** below are open explorations where I build and document these next-generation agent architectures.

[![Email](https://img.shields.io/badge/Email-kmjoo990929@gmail.com-blue?style=flat&logo=gmail)](mailto:kmjoo990929@gmail.com)
